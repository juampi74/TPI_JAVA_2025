...
<html>
  <head>
    ...
    <style>
      ...
      .nation-item .form-check-input:focus-visible,
      .nation-item .form-check-input:active {
        outline: none;
        box-shadow: none;
      }
  ...
  <body style="background-color: #10442e">
    ...
    <div class="container text-white">
      ...
      <p class="text-white-50">
        Tipo:
        <strong><%= association.getType().getDisplayName() %></strong>
        <%= association.getContinent() != null ? "(" +
        association.getContinent().getDisplayName() + ")" : "" %>
      </p>
      ...
      <form action="actionassociation" method="post" class="mt-4">
        ...
        <input
          type="hidden"
          name="id_association"
          value="<%= association.getId() %>"
        />

        <div
          class="countries-container mb-3"
          style="
            max-height: 400px;
            overflow-y: auto;
        ...
        >
          <div class="list-group">
            ...
            { boolean isMember = false; if (currentMembers != null) {
            for (Nationality member : currentMembers) { if
            (member.getId() == nationality.getId()) { isMember = true;
            break; } } } %>
            <label
              class="list-group-item d-flex align-items-center justify-content-between nation-item mb-2"
              for="chk_<%= nationality.getId() %>"
            >
              <span class="d-flex align-items-center">
                <% if(nationality.getFlagImage() != null) { %> <img
                class="flag-thumb mr-3"
                src="<%=request.getContextPath() + "/images?id=" +
                nationality.getFlagImage()%>" alt="flag <%=
                nationality.getName() %>"> <% } else { %>
                ...
                <span>
                  ...
                  <small class="text-muted"
                    ><%= nationality.getIsoCode() %></small
                  >
                </span>
              </span>
              <input class="form-check-input align-self-center"
              type="<%= inputType %>" name="ids_nationalities"
              value="<%= nationality.getId() %>" id="chk_<%=
              nationality.getId() %>" <%= isMember ? "checked" : "" %>
              />
            </label>
            <% } %> <% if (displayedNationalities.isEmpty()) { %>
            <div class="alert alert-warning">
              No hay nacionalidades cargadas para el continente <%=
              association.getContinent() %>.
            </div>
            <% } %>
          </div>
        </div>
        ...
        <div class="button-container">
          <a href="actionassociation" class="btn btn-dark border border-white">
		    Cancelar
		  </a>
          <button
            type="submit"
